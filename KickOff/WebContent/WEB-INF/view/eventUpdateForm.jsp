...
<map name="Map">
	...
	<form id= "eventupdate" action="eventUpdate" method="post" enctype="multipart/form-data">
	<input type="hidden" name="eventno" value="${eventUpdateForm.eventno}">
	    <center>
    <table cellpadding="4">
       <tr> 
         <th width = "200" align = "center">제목</th><td>  
         <input type = "text" id = "title" name = "title" size="100" value = "${eventUpdateForm.title}">
         </td>
       </tr>
       <tr>
         <th width = "200" align ="center">작성자</th><td>${eventUpdateForm.writer}</td>
       </tr>
       <tr> 
        	<th width = "200" align = "center">내용</th>
        	<td>
       			<c:choose>
				<c:when test="${eventUpdateForm.fileName != null}">
					  <img id="img1" src="/KickOff/img/${eventUpdateForm.fileName}" width="95%"><br>
					<input type="hidden" value="${eventUpdateForm.fileName}" name="fileName">
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${eventUpdateForm.fileName == null}">
         	...
         	<c:choose>	
				<c:when test="${eventUpdateForm.fileName2 != null}">
					<img id="img2" src="/KickOff/img/${eventUpdateForm.fileName2}" width = "95%"><br>
					<input type="hidden" value="${eventUpdateForm.fileName2}" name="fileName2">
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${eventUpdateForm.fileName2 == null}">
         	...
         	<c:choose>		
				<c:when test="${eventUpdateForm.fileName3 != null}">
					<img id="img3" src="/KickOff/img/${eventUpdateForm.fileName3}" width = "95%"><br>		
					<input type="hidden" value="${eventUpdateForm.fileName3}" name="fileName3">	
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${eventUpdateForm.fileName3 == null}">
			...
			<textarea rows="7" cols="100" id="contents" name = "contents" >${eventUpdateForm.contents}</textarea>
